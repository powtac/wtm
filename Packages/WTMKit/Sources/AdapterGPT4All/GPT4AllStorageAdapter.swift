...
/// Read-only GGUF inventory for an explicitly approved GPT4All models directory.
/// Directory names describe an import location, never repository or runtime ownership.
public struct GPT4AllStorageAdapter: StorageProviderAdapter {
  public let id = ProviderID.gpt4All
  public let displayName = "GPT4All"
      ...
      let task = Task {
        do {
          for try await entry in ReadOnlyDirectoryWalker().entryStream(
            ...
            guard
              ["gguf", "bin", "incomplete"].contains(entry.url.pathExtension.lowercased())
            else { continue }
            do {
              continuation.yield(try inspect(entry, source: source))
            } catch {
              continuation.yield(issue(source, url: entry.url, code: "GPT4ALL_FILE_UNVERIFIED"))
            }
          }
        } catch is CancellationError {
          // Cancelling an inventory is not an unreadable-file finding.
        } catch {
          continuation.yield(issue(source, url: source.rootURL, code: "GPT4ALL_SCAN_INCOMPLETE"))
        }
        continuation.finish()
      }
  ...
  private func inspect(_ entry: FileSystemEntry, source: ScanSource) throws -> AdapterScanBatch {
    ...
    let partial =
      entry.url.lastPathComponent.hasPrefix("incomplete-")
      || entry.url.pathExtension.lowercased() == "incomplete"
    let inspection =
      !partial && entry.url.pathExtension.lowercased() == "gguf"
    ...
    let identityID = "gpt4all:\(relativePath)"
    ...
    return AdapterScanBatch(
      ...
      issues: confirmed ? [] : issue(source, url: entry.url, code: "GPT4ALL_UNCONFIRMED").issues
    )
  }
    ...
    AdapterScanBatch(
      ...
      issues: [
        InventoryIssue(
          ...
          sourceID: source.id,
          summary:
            "This GPT4All source has an unverified file, layout, or completeness. Inventory may be incomplete.",
          affectedURL: url
        )
      ])
  }
